...
class CI_SHA1 {
	...
	{
		$n = ((strlen($str) + 8) >> 6) + 1;//O(1)

		for ($i = 0; $i < $n * 16; $i++)//O(16*n+1)
		{
			$x[$i] = 0;//O(1)
		}

		for ($i = 0; $i < strlen($str); $i++)//O(n+1)
		{
			$x[$i >> 2] |= ord(substr($str, $i, 1)) << (24 - ($i % 4) * 8);//O(1)
		}

		$x[$i >> 2] |= 0x80 << (24 - ($i % 4) * 8);//O(1)

		$x[$n * 16 - 1] = strlen($str) * 8;//O(1)
		...
		for ($i = 0; $i < count($x); $i += 16)//O(n+1)
		{
			...
			for ($j = 0; $j < 80; $j++)//O(n*(81))
			{
				...
				{
					$w[$j] = $this->_rol($w[$j - 3] ^ $w[$j - 8] ^ $w[$j - 14] ^ $w[$j - 16], 1);//O(1)
				}

				$t = $this->_safe_add($this->_safe_add($this->_rol($a, 5), $this->_ft($j, $b, $c, $d)), $this->_safe_add($this->_safe_add($e, $w[$j]), $this->_kt($j)));//O(1)

				$e = $d;//O(1)
				$d = $c;//O(1)
				$c = $this->_rol($b, 30);//O(1)
				$b = $a;//O(1)
				$a = $t;//O(1)
			}

			$a = $this->_safe_add($a, $olda);//O(1)
			$b = $this->_safe_add($b, $oldb);//O(1)
			$c = $this->_safe_add($c, $oldc);//O(1)
			$d = $this->_safe_add($d, $oldd);//O(1)
			$e = $this->_safe_add($e, $olde);//O(1)
		}

		return $this->_hex($a).$this->_hex($b).$this->_hex($c).$this->_hex($d).$this->_hex($e);//O(1)
	}
